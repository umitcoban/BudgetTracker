...
- `MonthlyBudgetCalculator` is the central aggregator: it combines salary rules, incomes, expenses, budgets, refund adjustments, statement rules, and delegates to `SubscriptionMonthlyCalculator`, `LoanMonthlyCalculator`, `FixedExpenseMonthlyCalculator`, and `CreditCardStatementCalculator`. Dashboard, Cash Flow, and Reports all derive from its `MonthlyBudgetSummary`, so a change to "what counts in a month" belongs there, not in a ViewModel.
