    ...
    suspend fun importFromJson(uri: Uri): ImportResult {
        return try {
            ...
            db.withTransaction {
                ...
                if (dto.schemaVersion >= 13) {
                    db.creditCardStatementRuleDao().insertAll(dto.creditCardStatementRules.map { it.toEntity() })
                }
...
private fun SalaryRuleDto.toEntity() = com.umit.budgettracker.core.database.entity.SalaryRuleEntity(
    id = id,
    amount = amount,
    effectiveStartMonth = effectiveStartMonth,
    note = note,
    payDay = payDay
)
...
private fun LoanDto.toEntity() = com.umit.budgettracker.core.database.entity.LoanEntity(
    id = id,
    title = title,
...
private fun CreditCardStatementRuleDto.toEntity() = com.umit.budgettracker.core.database.entity.CreditCardStatementRuleEntity(id, accountId, effectiveFromMonth, statementDay, dueDay)
...
private fun SubscriptionPriceHistoryDto.toEntity() = com.umit.budgettracker.core.database.entity.SubscriptionPriceHistoryEntity(
    id = id,
    subscriptionId = subscriptionId,
    amount = amount,
